...
public:
	...
	/*TODO: please make sure this transform is ok! */
    static struct dfa_graph_t * reassemble(sm &sm_)
    {
		...
		if(state_size < 65535 && graph){

			uchar  *alphabet_table = graph->alphabetTable;
			ushort *state = graph->nextTable;
			...
			throw runtime_error("DFA state too large!");
		}
